...
This code includes the `low_memory=False` parameter in the first call to `detect_multilingual`, adds the second call to `detect_multilingual` with the same input string after the comment showing the example output, and ensures the order and format match the gold code.